...
class FakeLocal implements WalkMediaLocalDataSource {
  ...
  @override
  Future<List<WalkMediaModel>> getWalkMediaByWalkId(int walkId) async => items.where((e) => e.walkId == walkId).toList();
  @override
...
class FakeRemoteOk implements WalkMediaRemoteDataSource {
  ...
  @override
  Future<List<WalkMediaModel>> getWalkMediaByWalkId(int walkId) async => const [];
  @override
...
class FakeRemoteFail implements WalkMediaRemoteDataSource {
  @override
  Future<int> addWalkMedia(WalkMediaModel walkMediaModel) async => throw ServerException();
  @override
  Future<int> deleteWalkMedia(int userId) async => throw ServerException();
  @override
  Future<List<WalkMediaModel>> getWalkMediaByWalkId(int walkId) async => throw ServerException();
  @override
  Future<List<WalkMediaModel>> getWalkMedias() async => throw ServerException();
  @override
  Future<int> updateWalkMedia(WalkMediaModel walkMediaModel) async => throw ServerException();
}

void main() {
  test('online returns Right', () async {
    final repo = WalkMediaRepositoryImpl(walkMediaLocalDataSource: FakeLocal(), walkMediaRemoteDataSource: FakeRemoteOk(), networkInfo: FakeNet(true));
    final res = await repo.getWalkMedia();
    expect(res, isNotNull);
    res!.fold((l) => fail('Expected Right'), (r) => expect(r, []));
    expect(await repo.addWalkMedia(const WalkMediaModel(id: 1, walkId: 2, userId: 3, mediaUrl: 'u')), const Right(1));
  });
  test('offline returns local', () async {
    final local = FakeLocal()..items = const [WalkMediaModel(id: 1, walkId: 2, userId: 3, mediaUrl: 'u')];
    final repo = WalkMediaRepositoryImpl(walkMediaLocalDataSource: local, walkMediaRemoteDataSource: FakeRemoteOk(), networkInfo: FakeNet(false));
    final res = await repo.getWalkMediaByWalkId(2);
    expect(res!.isRight(), true);
  });
  test('remote failures map Left', () async {
    final repo = WalkMediaRepositoryImpl(walkMediaLocalDataSource: FakeLocal(), walkMediaRemoteDataSource: FakeRemoteFail(), networkInfo: FakeNet(true));
