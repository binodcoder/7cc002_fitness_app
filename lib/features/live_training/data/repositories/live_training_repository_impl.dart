...
class LiveTrainingRepositoryImpl implements LiveTrainingRepository {
  ...
  @override
  Future<Either<Failure, int>>? addLiveTraining(entity.LiveTraining liveTraining) async {
    try {
      final model = liveTraining is LiveTrainingModel
          ? liveTraining as LiveTrainingModel
  ...
  @override
  Future<Either<Failure, int>>? updateLiveTraining(entity.LiveTraining liveTraining) async {
    try {
      final model = liveTraining is LiveTrainingModel
          ? liveTraining as LiveTrainingModel
          : LiveTrainingModel.fromEntity(liveTraining);
      int response = await liveTrainingRemoteDataSource.updateLiveTraining(model);
      return Right(response);
    } on ServerException {
      return Left(ServerFailure());
    }
